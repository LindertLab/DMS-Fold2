...
from openfold.utils.dmsfold_utils_subsample import make_dms_tensor
...
def main(args):
    ...
    for i in range(len(dms_list)):
        ...
        if not os.path.isfile(sm_dms_path):
            ...
            sys.exit(1)
        elif not os.path.isfile(dm_dms_path):
            print(f"Missing DM DMS file at path: {dm_dms_path}")
            sys.exit(1)
        try:
            dms_tensor = make_dms_tensor(sm_dms_path,dm_dms_path,seq_list[i][0],tag_list[i][1][0])
            dms_values = torch.nan_to_num(dms_tensor, nan=0.0, posinf=0.0, neginf=0.0)
            dms_mask = (dms_values != 0).float()
            dms_list.append((dms_values,dms_mask))
        except Exception as e:
            print(f"[ERROR] Failed to load DMS for {tag_list[i][1][0]}: {e}")
            traceback.print_exc()
            sys.exit(1)
    ...
    for model, output_directory in model_generator:
        ...
        for (tag, tags), seqs, dms_enrichment in sorted_targets:
            ...
            if feature_dict is None:
                ...
                feature_dicts[tag]["dms_values"] = dms_enrichment[0]
                feature_dicts[tag]["dms_mask"] = dms_enrichment[1]
                msa = feature_dicts[tag]['msa']
                ...
                if args.neff:
                    ...
                    for item in feature_dicts[tag]:
                        print(item)
                    feature_dicts[tag]['msa'] = msa[indices]
                    feature_dicts[tag]['deletion_matrix_int'] = feature_dicts[tag]['deletion_matrix_int'][indices]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "dms_dir", type=str,
        help="Path to directory containing DMS csv files, one file per sequence"
    )
    parser.add_argument(
